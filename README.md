...
这是一个以 Markdown 为唯一内容源的中文长文 Agent Skill，兼容 Codex、Claude Code 以及其他支持 [Agent Skills](https://agentskills.io) 格式的智能体（下文统称 Agent）。它可以从一个选题或方向开始，完成文章正文、配图计划、图片生成和 HTML 工作台打包，并在明确要求时把同一篇终稿同步到微信、今日头条和小红书草稿。
...
该 skill 使用开放的 Agent Skills 格式，可安装到任何支持该格式的 Agent。
...
将 `wechat-article-pipeline/` 复制到对应 Agent 的 skills 目录即可。`agents/openai.yaml` 是 Codex 专用的界面元数据，其他 Agent 会自动忽略。

## 三、使用方法

在 Agent 中说明你要写的公众号文章主题、方向或已有初稿即可。默认流程会生成一套可本地审核和编辑的文章包：
...
1. 你先用其他大模型或自己写好初稿，再交给 Agent 使用该 skill 排版、配图和打包。
2. 你只给 Agent 一个选题、观点或文章方向，由 Agent 使用该 skill 直接完成初稿。
...
秘书模式不会自行扩大主题、补写一套更完整的观点、增加平衡框架，或把文章改造成更像报告的 AI 大纲。原素材存在关键缺口时，Agent 会指出缺口或询问，而不是替你发明新的主张。

### 配图方式

配图也可以来自两种方式：

1. 使用其他图片模型先生成图片，再把图片文件交给 Agent 打包进 HTML。
2. 由 Agent 根据文章内容自动规划题图、正文配图和尾图，并调用可用的图片生成能力完成配图。
...
拿到 `AppID` 和 `AppSecret` 后，把这两个值告诉 Agent，并说明公众号名称。Agent 会在本机生成本地配置文件；这个文件只保存在本机，不应提交到 GitHub，也不要贴到公开聊天或文档里。

如果只绑定一个公众号，后续导入草稿时通常不需要再指定公众号名称。如果绑定了多个公众号，导入草稿前需要告诉 Agent 要导入到哪一个公众号。
...
- `V 1.7.0（当前版本）`：工作台支持在右侧微信预览中点击段落之间直接粘贴图片，自动把图片保存到文章目录并写回 Markdown；Skill 的界面显示名与调用名统一为 `wechat-article-pipeline`，并汇总 1.6.0 之后的草稿修订、发布确认、表格保留和白名单错误处理修复。
...
This Agent Skill uses Markdown as the sole source for Chinese long-form content. It is compatible with Codex, Claude Code, and any other agent that supports the open [Agent Skills](https://agentskills.io) format (referred to as "the agent" below). Starting from a topic or direction, it can write the article, plan and generate visuals, package an editable HTML workbench, and, when explicitly requested, synchronize the approved article to WeChat, Toutiao, and Xiaohongshu drafts.
...
The skill uses the open Agent Skills format and can be installed into any agent that supports it.
...
Copy the skill directory into the personal Claude Code skills directory:
...
Copy `wechat-article-pipeline/` into the agent's skills directory. `agents/openai.yaml` is Codex-specific interface metadata; other agents ignore it.

## 3. Usage

Describe to the agent the WeChat Official Account article topic, direction, or draft you already have. By default, the skill produces a local package for review and editing:
...
1. Write or draft it elsewhere first, then ask the agent to use this skill for editing, image planning, and packaging.
2. Give the agent only a topic, point of view, or direction, and let it draft the article with this skill.
...
1. Generate images with another image model first, then give the files to the agent for packaging.
2. Let the agent plan and generate the cover image, in-body images, and closing image from the article content.
...
After you have the `AppID` and `AppSecret`, give them to the agent and include the Official Account name. The agent will create a local config file on this machine. Keep that file local; do not commit it to GitHub or paste it into public chats or documents.

If only one Official Account is bound, future draft imports usually do not need an account name. If multiple accounts are bound, tell the agent which Official Account to use before importing a draft.
...
When importing a draft, the agent will:
...
- `V 1.7.0 (current version)`: Added direct image pasting between blocks in the WeChat preview, with automatic asset storage and Markdown insertion; aligned the UI display name with `wechat-article-pipeline`, and rolled up post-1.6.0 fixes for draft revisions, publish confirmation, table preservation, and IP-allowlist failures.
