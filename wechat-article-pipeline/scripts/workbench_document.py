...
from atomic_files import locked_file, atomic_replace, atomic_write_bytes, atomic_write_text, fsync_directory
...
class WorkbenchDocument:
    ...
    def _refresh_manifest(self, req: ManifestRefreshRequest) -> tuple[bool, str]:
        ...
        candidate = req.manifest_path.with_name(req.manifest_path.name + f".r{req.revision}.{self.token[:12]}.candidate")
        command = [
            sys.executable,
            str(MAKE_MANIFEST),
            ...
            while True:
                ...
                try:
                    ...
                    current.manifest_path.with_name(
                        current.manifest_path.name + f".r{current.revision}.{self.token[:12]}.candidate"
