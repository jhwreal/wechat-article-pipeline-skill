...
If that same conversation creates another draft for the same article slug, treat it as a revision and rebuild the package with `--same-session-revision`. Packaging reads the issue from this article's existing job signature and writes `counter_policy=reuse_previous`. It checks the article and account binding; if the signature is missing, recover the actual prior job instead of guessing from the account counter. Before upload, the publisher requires the current counter to be greater than that already consumed issue; after success it leaves `.env` unchanged. Use this flag only when both the current agent conversation and article slug match a prior successful draft. Do not infer revision state merely from old receipt files in the workspace, and do not manually roll the counter backward for revisions.
